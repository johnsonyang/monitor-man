const router = new (require('koa-router'))({prefix: '/films'});
  // , fs = require('fs')
...
// get all collections
// http://localhost:8889/films/
router.get('/', async function (ctx) {
  // 下面的信息来自jwt的内容,参见/Users/Johnson/2020study/monitor-man/routes/api/users.js
  ...
  // }
  // console.log(ctx.request.usuarioId);
  ctx.response.body = await Film.findAll();
...
router.post('/', async function (ctx) {
  ...
  ctx.response.body = await Film.create(ctx.request.body);
});
